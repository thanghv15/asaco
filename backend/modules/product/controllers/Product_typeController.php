...
class Product_typeController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($image) {
                $image->saveAs('../../public/images/product_type/product_type-'.$time_curent.'.'. $image->extension );
                $model->image_preset = 'product_type-'.$time_curent.'.'.$image->extension;
            }
            if ($model->parent_id) {
            ...
            $log = new Logfile();
            $arr = array();
            $messages = 'Tạo mới loại sản phẩm';
            $resource = 'Sản phẩm';
            $level = 3;
            array_push($arr, $messages);
            array_push($arr, $level);
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $log = new Logfile();
            $arr = array();
            $messages = 'Chỉnh sửa loại sản phẩm';
            $resource = 'Sản phẩm';
            $level = 3;
            array_push($arr, $messages);
            array_push($arr, $level);
